...
import (
	...
	redis "github.com/lz006/ceph_exporter/collectors/rediscom"
)
...
type PoolUsageCollector struct {
	...
	// Used bytes of an image.
	ImageUsedBytes *prometheus.GaugeVec
}
...
func NewPoolUsageCollector(conn Conn, cluster string) *PoolUsageCollector {
	var (
		...
		poolLabelImgages = []string{"pool", "image", "size"}
	)
	...
	return &PoolUsageCollector{
		...
		ImageUsedBytes: prometheus.NewGaugeVec(
			prometheus.GaugeOpts{
				Namespace:   cephNamespace,
				Subsystem:   subSystem,
				Name:        "image_used_bytes",
				Help:        "Total bytes that are in use for a particular image",
				ConstLabels: labels,
			},
			poolLabelImgages,
		),
...
func (p *PoolUsageCollector) collect() error {
	...
	for _, pool := range stats.Pools {
		...
		images, err := redis.GetImagesFromRedis(pool.Name)
		if err == nil {
			for _, image := range images {
				p.ImageUsedBytes.With(prometheus.Labels{
					"pool":  pool.Name,
					"image": (*image).Name,
					"size":  fmt.Sprintf("%f", (*image).Size),
				}).Set((*image).Used)
			}
		}
	}
